...
@Deprecated
public class RampSection extends DiagramComponent {
	/**
	...
	public RampSection(Coordinate location, Dimension length, Direction direction) {
		super(location);
	...
	public Coordinate getSize() {
		...
		case LEFT:
		case RIGHT:
			return new Coordinate(this.getLength(), RampSection.DEFAULT_WIDTH);

		case UP:
		case DOWN:
			return new Coordinate(RampSection.DEFAULT_WIDTH, this.getLength());
	...
	public Landing newLanding(Coordinate size, Dimension offset) {
		...
		switch (this.direction) {
		case LEFT:
			location.add(new Coordinate(new Dimension(0), size.getY().clone().scale(-0.5).add(RampSection.DEFAULT_WIDTH.clone().scale(0.5))));
			location.getY().add(offset);
			break;
		case RIGHT:
			location.add(new Coordinate(this.getLength(), size.getY().clone().scale(-0.5).add(RampSection.DEFAULT_WIDTH.clone().scale(0.5))));
			location.getY().add(offset);
			break;
		case UP:
			location.add(new Coordinate(size.getX().clone().scale(-0.5).add(RampSection.DEFAULT_WIDTH.clone().scale(0.5)), new Dimension(0)));
			location.getX().add(offset);
			break;
		default:
			location.add(new Coordinate(size.getX().clone().scale(-0.5).add(RampSection.DEFAULT_WIDTH.clone().scale(0.5)), this.getLength()));
			location.getX().add(offset);
			break;
		}
...
//		double idealDist = Ramp.DEFAULT_MIN_POST_DISTANCE.getSum(Ramp.DEFAULT_MAX_POST_DISTANCE).getLength() / 2.0;
		...
		int spaceCount = (int) Math.ceil(insideLength / RampSection.DEFAULT_MAX_POST_DISTANCE.getLength());
		...
		for (int i = 0; i < posts.length; i++) {
			...
			if (i == posts.length / 2) {
				...
				pU = RampSection.DEFAULT_WIDTH.getLength();
				pV = 0;
			}
